...
Slika *radonova_transformacija(Slika *img)
{
    ...
    sinogram->data = (Pixel *)calloc(2*ro_max*najveci_teta,sizeof(Pixel));
    if (sinogram->data == NULL) {
        printf("Greska pri alociranju memorije!\n");
        exit(4);
    }
    sinogram->x = ro_max;
    sinogram->y = najveci_teta;


    #pragma omp parallel for shared(sinogram,najveci_teta,ro_max,ro_poluprecnik,centar_x,centar_y)
    for(int teta = 1; teta < 45; teta++) {
        ...
        for (int ro = 1; ro < ro_max; ro++) {
            ...
            for(int y = ymin; y < ymax; y++) {
                ...
                if(sinogram->data[ro_max - ro - 1 + (najveci_teta - (int)teta) * ro_max].blue + img->data[((int)y+centar_x)*img->x + (int)x + centar_y ].blue <=255)
                {
                sinogram->data[ro_max - ro - 1 + (najveci_teta - (int)teta) * ro_max].blue += img->data[((int)y+centar_x)*img->x + (int)x + centar_y ].blue/12;
                }
                if(sinogram->data[ro_max - ro - 1 + (najveci_teta - (int)teta) * ro_max].green + img->data[((int)y+centar_x)*img->x + (int)x + centar_y ].green <=255)
                {
                sinogram->data[ro_max - ro - 1 + (najveci_teta - (int)teta) * ro_max].green += img->data[((int)y+centar_x)*img->x + (int)x + centar_y].green/12;
                }
                if(sinogram->data[ro_max - ro - 1 + (najveci_teta - (int)teta) * ro_max].red + img->data[((int)y+centar_x)*img->x + (int)x + centar_y ].red <=255)
                {
                sinogram->data[ro_max - ro - 1 + (najveci_teta - (int)teta) * ro_max].red += img->data[((int)y+centar_x)*img->x + (int)x + centar_y].red/12;
                }
            }
        }
    }
